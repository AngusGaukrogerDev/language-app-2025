...
export const AppwriteTest = () => {
  ...
  return (
    <div className="p-4 sm:p-6 bg-white rounded-lg shadow-sm border border-gray-200">
      <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-4">Appwrite Connection Test</h3>
      <button
        onClick={testConnection}
        disabled={loading}
        className="bg-gray-900 text-white px-4 py-3 sm:py-2 rounded-md text-base sm:text-sm font-medium hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px]"
      >
        {loading ? 'Testing...' : 'Test Connection'}
      </button>
      {testResult && (
        <div className="mt-4 p-3 sm:p-4 bg-gray-50 rounded-md">
          <p className="text-sm sm:text-base text-gray-700 break-words">{testResult}</p>
        </div>
      )}
    </div>
  );
